<x-layout>
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Modifier un Profil</h1>
        ...
        <form action="{{ route('profiles.update', $profile) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="lastname" name="lastname" 
                    ...
                    value="{{ old('lastname', $profile->lastname) }}" required>
                @error('lastname')
            ...
            <div class="mb-4">
                ...
                <input type="text" id="firstname" name="firstname" 
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    value="{{ old('firstname', $profile->firstname) }}" required>
                @error('firstname')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" id="job" name="job" 
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    value="{{ old('job', $profile->job) }}" required>
                @error('job')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="experiences" name="experiences" 
                    class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    rows="4">{{ old('experiences', $profile->experiences) }}</textarea>
                @error('experiences')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
